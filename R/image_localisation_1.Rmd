...
annot_file <- "G:/@ModAn/ImgRecSelfAssess/training_data_processed/export-2019-03-04T09_30_59.776Z.json"
annot_coco_file <- "G:/@ModAn/ImgRecSelfAssess/training_data_processed/export-coco-2019-03-05T14_31_52.619981.json"
annot_csv <- "G:/@ModAn/ImgRecSelfAssess/training_data_processed/export-2019-02-13T12_03_36.744Z.csv"
```

Importing image attributes from csv and json
```{r}
#import and tidy csv file
annotations <- read.csv(annot_csv)

annotations <- annotations %>% 
  select(ID, External.ID, Label, Created.By, Dataset.Name) %>% 
  filter(Label != "Skip") %>%  #exclude skipped images
  mutate(Label = as.character(Label)) %>% #convert Label to character vector
  mutate(Label = map(.$Label, fromJSON)) %>% 
  droplevels()
annotations
```

```{r}
#process json component

temp <- annotations$Label[12][[1]]
unlist(temp)

#jsonedit(temp)
#map(temp, "object")
map(temp[[1]], unlist)
unlist(temp[[1]], recursive = F)
```

###Importing image attributes from json coco
```{r}
#Image attributes - JSON

#image annotations
annotations <- fromJSON(file = annot_coco_file)
#str(annotations[150], max.level = 2)
#head(annotations[150])
#jsonedit(annotations)
```




###Importing image attributes from json only
```{r}
#Image attributes - JSON
...
annotations_df <- annotations %>%
    set_names(map_chr(., "External ID")) %>% 
    enframe("filename", "annotations")
annotations_df
...
Try to flatten a single image record

```{r}
one_image <- annotations_df$annotations[[180]]
...
```{r}
labelnames <- map_chr(one_image, c())
  
one_image %>%
    set_names(map(., "Label")) %>% 
    enframe("Label", "annotations")
```
...
```{r}
one_label <- one_image[["Label"]]
jsonedit(one_label)

map_df(one_image, magrittr::extract, c("Label"))
```
...
annotations_df %>% 
   mutate(
     info = annotations %>%
       map(. %>%
             map_df(magrittr::extract, c("Label", "Created By", "Dataset Name")))
   ) %>% 
   select(-annotations) %>% 
   tidyr::unnest()
...
imageinfo <- annotations %>% {
  tibble(
    id = map_chr(., "ID"),
    file_name = map_chr(., "External ID"),
    dataset_name = map_chr(., "Dataset Name")
  )
}

table(imageinfo$dataset_name) 
```
